...
class BulaGraphImporter:
    ...
    def import_leaflet(
        ...
        from utils.process import detectar_apresentacao
        apresentacao = detectar_apresentacao(text)
        formulation = apresentacao
        ...
        # ---------------------------------------------------------------
        for order, (raw_title, section_type, section_text, start_pos) in enumerate(sections):
            # Nó Section
            ...
            for chunk_idx, chunk_text in enumerate(chunks):
                ...
                # Calcula posições globais relativas ao texto completo da bula
                global_start_char = start_pos + start_char
                global_end_char = start_pos + end_char
                
                # Tenta extrair subtítulos de markdown no chunk (ex: ## Subtítulo)
                subheadings = re.findall(r"^#+\s+(.+)$", chunk_text, re.MULTILINE)
                section_heading_path = [raw_title] + subheadings
                
                # Source files
                source_pdf = source if source.lower().endswith(".pdf") else f"bula_{medication_name.lower()}.pdf"
                source_markdown = source if source.lower().endswith(".md") else f"bula_{medication_name.lower()}_cleaned.md"
    ...
    def _detect_sections(
        ...
        Returns:
            Lista de (raw_title, section_type, section_text, start_pos)
        """
        ...
        for i, (start_pos, raw_title, section_type) in enumerate(matches):
            ...
            if section_text:
                sections.append((raw_title, section_type, section_text, start_pos))
