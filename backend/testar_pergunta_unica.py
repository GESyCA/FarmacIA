...
def main():
    ...
    parser.add_argument("--medicamento", type=str, default="AMOXIL", help="Nome do medicamento")
    parser.add_argument("--pergunta", type=str, default="Para que serve o Amoxil?", help="Pergunta a ser feita")
    parser.add_argument("--resposta_esperada", type=str, default="Amoxil é um antibiótico indicado para o tratamento de infecções bacterianas causadas por germes sensíveis à ação da amoxicilina.", help="Resposta esperada (gabarito) para avaliação")
    ...
    configs = [
        ...
        # ("Guarded Hybrid Agentic Fusion RAG", "configs/exp_compare_07_guarded.yaml"),
    ]
    ...
    # Monta o nome do arquivo com: LLM gerador + embedding + nível de dificuldade
    gen_llm_raw = args.generation_llm or (first_config['models']['generation_llm'] if first_config else "unknown")
    gen_llm_clean = gen_llm_raw.replace("ollama:", "").replace("llama_cpp:", "")
    gen_llm_clean = re.sub(r'[^a-zA-Z0-9_]', '_', gen_llm_clean).lower()
    gen_llm_clean = re.sub(r'_+', '_', gen_llm_clean).strip('_')
    
    emb_name_raw = first_config['models']['embedding_model'] if first_config else "unknown"
    emb_name_clean = re.sub(r'[^a-zA-Z0-9_]', '_', emb_name_raw).lower()
    emb_name_clean = re.sub(r'_+', '_', emb_name_clean).strip('_')
    
    difficulty_clean = args.difficulty.lower()
    
    csv_output_path = f"resultados/pergunta_unica_todos_pipelines_{gen_llm_clean}_{emb_name_clean}_{difficulty_clean}_output.csv"
