...
json_path = os.path.join(BASE_DIR, "medgemma_4b", "medgemma_compiled_results.json")
local_artifact_path = os.path.join(BASE_DIR, "medgemma_4b_comparison.md")
...
if brain_dir:
    brain_artifact_path = os.path.join(brain_dir, "medgemma_4b_comparison.md")
else:
    brain_artifact_path = r"C:\Users\silvi\.gemini\antigravity-ide\brain\1a3c785a-6072-4558-9ec6-db9b3d651509\medgemma_4b_comparison.md"
...
md.append("")
md.append("Este relatório compara de forma objetiva o impacto do **nível de dificuldade das perguntas** e do **pipeline de RAG** na recuperação e na geração de respostas, além de reportar os tempos médios de execução obtidos com o modelo **MedGemma 4b**.")
md.append("")
md.append("> [!NOTE]")
...
for pipe in pipelines:
    ...
    for mkey, _ in table_metrics:
        # Ignore section metrics for Naive RAG as they are structural and intentionally zero
        val_list = []
        for diff in difficulties:
            key = f"{pipe}|{diff}"
            metrics = data.get(key, {})
