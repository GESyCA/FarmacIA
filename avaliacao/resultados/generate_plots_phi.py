...
# Color palettes
comparison_palette = ["#E67E22", "#107C41"]  # Phi4 orange vs MediPhi green

# Paths
base_dir = os.path.dirname(os.path.abspath(__file__))
phi4_json = os.path.join(base_dir, "phi4-mini", "phi4_compiled_results.json")
mediphi_json = os.path.join(base_dir, "mediphi", "mediphi_compiled_results.json")
output_plots_dir = os.path.join(base_dir, "graficos")

os.makedirs(output_plots_dir, exist_ok=True)

# Load data
if not os.path.exists(phi4_json):
    print(f"Warning: {phi4_json} not found.")
    phi4_data = {}
else:
    with open(phi4_json, 'r', encoding='utf-8') as f:
        phi4_data = json.load(f)

if not os.path.exists(mediphi_json):
    print(f"Warning: {mediphi_json} not found.")
    mediphi_data = {}
else:
    with open(mediphi_json, 'r', encoding='utf-8') as f:
        mediphi_data = json.load(f)
...
df_phi4 = build_df(phi4_data, "Phi4 Mini")
df_mediphi = build_df(mediphi_data, "MediPhi Instruct")
df_all = pd.concat([df_phi4, df_mediphi], ignore_index=True)

# Save dataframes as csv for record
if not df_all.empty:
    df_all.to_csv(os.path.join(output_plots_dir, "phi_compiled_data.csv"), index=False, encoding='utf-8')

# ----------------------------------------------------
# 2. Phi4 Mini Intra-model plots
# ----------------------------------------------------
if not df_phi4.empty:
    ...
    plt.title("Phi4 Mini - Final Score por Pipeline e Dificuldade", pad=15)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "phi4_final_scores.png"), dpi=300)
    plt.close()
    ...
    plt.title("Phi4 Mini - Taxa de Falhas Críticas (%)", pad=15)
    ...
    plt.legend(title="Dificuldade", bbox_to_anchor=(1.02, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "phi4_critical_failures.png"), dpi=300)
    plt.close()

# ----------------------------------------------------
# 3. MediPhi Instruct Intra-model plots
# ----------------------------------------------------
if not df_mediphi.empty:
    # 3.1 Final Score
    plt.figure(figsize=(10, 6))
    ax = sns.barplot(
        data=df_mediphi,
    ...
    plt.title("MediPhi Instruct - Final Score por Pipeline e Dificuldade", pad=15)
    plt.ylabel("Final Score (Média, escala 0-1)")
    plt.xlabel("Pipeline RAG")
    plt.ylim(0, 1.05)
    label_bars(ax, 'score')
    plt.legend(title="Dificuldade", bbox_to_anchor=(1.02, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "mediphi_final_scores.png"), dpi=300)
    plt.close()
    ...
    plt.title("MediPhi Instruct - Taxa de Falhas Críticas (%)", pad=15)
    plt.ylabel("Falhas Críticas (%)")
    plt.xlabel("Pipeline RAG")
    plt.ylim(0, 105)
    label_bars(ax, 'percent')
    plt.legend(title="Dificuldade", bbox_to_anchor=(1.02, 1), loc='upper left')
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "mediphi_critical_failures.png"), dpi=300)
    plt.close()

# ----------------------------------------------------
# 4. Direct Model Comparison (Phi4 Mini vs MediPhi Instruct)
# ----------------------------------------------------
if not df_all.empty:
    ...
    plt.title("Comparação Consolidada: Phi4 Mini vs MediPhi Instruct (Média Final Score)", pad=15)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "comparison_phi_final_scores_consolidated.png"), dpi=300)
    plt.close()
    ...
    plt.title("Comparação Consolidada: Phi4 Mini vs MediPhi Instruct (Taxa de Falhas Críticas)", pad=15)
    ...
    plt.legend(title="Modelo")
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "comparison_phi_critical_failures_consolidated.png"), dpi=300)
    plt.close()
    ...
    plt.title("Comparação Consolidada (Phi): Tempo Médio de Recuperação (s)", pad=15)
    plt.ylabel("Tempo de Recuperação (segundos)")
    plt.xlabel("Pipeline RAG")
    label_bars(ax, 'time')
    plt.legend(title="Modelo")
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "comparison_phi_retrieval_time_consolidated.png"), dpi=300)
    plt.close()
    ...
    plt.title("Comparação Consolidada (Phi): Tempo Médio de Inferência (s)", pad=15)
    plt.ylabel("Tempo de Inferência (segundos)")
    plt.xlabel("Pipeline RAG")
    label_bars(ax, 'time')
    plt.legend(title="Modelo")
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "comparison_phi_inference_time_consolidated.png"), dpi=300)
    plt.close()
    ...
    plt.title("Comparação Consolidada (Phi): Média de Chunk Recall@3", pad=15)
    ...
    plt.legend(title="Modelo")
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "comparison_phi_chunk_recall_3_consolidated.png"), dpi=300)
    plt.close()
    ...
    plt.title("Comparação Consolidada (Phi): Média de Chunk Recall@10", pad=15)
    ...
    plt.legend(title="Modelo")
    plt.tight_layout()
    plt.savefig(os.path.join(output_plots_dir, "comparison_phi_chunk_recall_10_consolidated.png"), dpi=300)
    plt.close()

print("All Phi plots successfully generated!")
